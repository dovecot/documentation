...
-  Create ``/var/log/dovecot.log`` and ``/var/log/dovecot-info.log`` files owned by vmail:vmail, so that `dovecot-lda <https://wiki2.dovecot.org/LDA>`_ can write to them.
...
You can configure the SMTP server to deliver mails internally, or you can use `dovecot-lda <https://wiki2.dovecot.org/LDA>`_.
