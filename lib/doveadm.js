...
function createTypeProxy(target, name) {
	return new Proxy(target, {
		get(target, prop) {
			if (prop in target || typeof prop === 'symbol') {
				return target[prop]
			}
			const err = new Error(`Invalid ${name} key: ${String(prop)}`)
...
export const doveadm_arg_types = createTypeProxy({
	ARRAY:			1,
	BOOL:			2,
	INTEGER:		3,
	STRING:			4,
...
export const doveadm_response_types = createTypeProxy({
	STRING:		'string',
	INTEGER:	'integer',
	TIMESTAMP:	'timestamp',
}, 'doveadm_response_types')
...
export function getDoveadmCmdLine(args) {
	...
	for (const [k, v] of Object.entries(args)) {
		...
		if (v.positional) {
			ret += (v.optional ? '[' : '<') + k.replaceAll('-', ' ') + (v.optional ? '] ' : '> ')
		} else {
			ret += '[-'
			if (v.cli) {
				ret += v.cli
...
export async function normalizeDoveadm(doveadm) {
	...
	for (const [k, v] of Object.entries(doveadm)) {
		...
		/* Response values.
		 * response: null for no output, or { note, fields, example }. */
		if (v.response && typeof v.response === 'object') {
			if (v.response.note) {
				v.response.note = md.render(v.response.note)
			}

			if (v.response.fields) {
				v.response.fields = Object.entries(v.response.fields).map(([name, field]) => ({
					name,
					...field,
					description: field.description ? md.renderInline(field.description) : null
				}))
			}
		}
