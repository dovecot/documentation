...
The marker (`END` above) can be any string that does not appear as a
standalone line within the value. The terminating marker line must match
exactly — any leading or trailing whitespace will prevent it from being
recognized. Each content line is included in the value as-is, including a
trailing newline.

Heredoc is only supported for string and file settings. Using it for list,
boolean, or numeric settings results in an error.
