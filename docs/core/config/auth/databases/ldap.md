...
```[dovecot.conf]
  passdb_ldap_bind = yes
  passdb_ldap_bind_userdn = cn=%u,ou=people,o=org
```
:::
...
See the `ssl_*` settings in `dovecot-ldap-example.conf` for how to
configure TLS.
...
specified in the `ssl_CACERTDIR` option found under `/etc/openldap/ldap.conf`
...
(the user which is specified with `ldap_auth_dn` parameter) read access to
`userPassword` field in the LDAP server.

An example of this: assuming that the user assigned to `ldap_auth_dn` is
...
If `use_workers=yes`, auth worker processes are used to perform the lookups.

Each auth worker process creates its own LDAP connection so this can
increase parallelism.

With `use_workers=no`, the auth master process can keep 8 requests pipelined
for the LDAP connection, while with `use_workers=yes` each connection has a
maximum of 1 request running.

For small systems, `use_workers=no` is sufficient and uses less resources.
...
### `ldap_iterate_fields`
- Default: `<empty>`
- Values: [[link,settings_types_string_list]]
- *LDAP Authentication Only*
...
Example: `auth_ldap_uris = ldaps://secure.domain.org`
