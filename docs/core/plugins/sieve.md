...
dovecotlinks:
  ...
  sieve_storage:
    ...
    text: Script storage
  sieve_storage_type:
    hash: script-storage-type
    text: Script storage types
  sieve_storage_driver:
    hash: script-storage-driver
    text: Script storage drivers
  sieve_storage_file:
    hash: script-storage-file
    text: File storage driver
  sieve_storage_dict:
    hash: script-storage-dict
    text: Dict storage driver
  sieve_storage_ldap:
    hash: script-storage-ldap
...
repeating the sieve_script block and adding additional configuration settings -
or it allows userdb to override storage settings for specific users
(FIXME: example?).
...
script, personal scripts can also be retrieved by name. The Sieve include
extension will then use this storage for retrieving `:personal` scripts and the
ManageSieve service will be able to store the user's scripts there.
...
message are executed. This "discard script" can prevent discarding the message,
by executing alternative actions. If the discard script does nothing,
the message is still discarded as it would be when no discard script is
configured.

#### `global`

An `global` storage is the source of `:global` include scripts for the Sieve
...
setting. If left unconfigured for a [[setting,sieve_script]] block, the storage
driver is `file` and the default directory is `~/sieve/` with a symbolic link
at `~/.dovecot.sieve` pointing to the active script file in that directory.
...
system. This is the default type if the `sieve_script_driver` setting is
omitted.
...
When a script storage with type `personal` is using the `file` driver and the
[[setting,sieve_script_path]] points to a directory, a symbolic link points to
the currently active script (the script executed at delivery). The active script
can be modified by the user through ManageSieve and by the administrator using
`doveadm sieve`. The location of this symbolic link can be configured
using the [[setting,sieve_script_active_path]] setting.

When a script storage with type `before` or `after` is using the `file` driver
...
[[setting,sieve_script_ldap_mod_attr]] to detect the need to compile. Therefore,
if a script is changed, then this entry must also be changed for it to be
reloaded. Depending on which LDAP entry is configured, this can happen
...
sieve_script personal {
  ...
  ldap_dn = cn=sieve,ou=Programs,dc=example,dc=org
  ...
  ldap_script_attr = mailSieveRuleSource

  # Attribute used for modification tracking
  ldap_mod_attr = modifyTimestamp
}
```
:::
...
by configuring a `personal` script storage (see [[link,sieve_storage]] above).
This can be done in two ways:

1. Define the full `sieve_script` block in dovecot configuration.
...
policies before the user's script. See the `before` and `after` Sieve storage
types [[link,sieve_storage_type]] for details on how to configure the execution
sequence.
...
::: tip
Be sure to manually pre-compile the scripts specified by `before` and `after`
Sieve storage types by using the [[link,sievec]] tool.
:::

## Visible Default Script

The `default` Sieve storage type [[link,sieve_storage_type]] specifies the
location of a default script that is executed when the user has no active
personal script.
...
`default` storage setting. The `default` storage needs to point to a valid
