...
<!-- @include: include/2.4.6-boolean-values.inc -->

<!-- @include: include/2.4.6-path-types.inc -->

<!-- @include: include/2.4.6-search-nesting-limit.inc -->

<!-- @include: include/2.4.6-shutdown-clients.inc -->
...
| Feature | Notes |
| ------- | ----- |
| [[setting,service_shutdown_clients_timeout]] | Keep the existing sessions running for a while after [[doveadm,reload]], e.g. to take new SSL certificates into use without disconnecting anyone. See [[link,running_reload]]. |
| [[doveadm,reload,--kick-timeout]] | Override [[setting,service_shutdown_clients_timeout]] for a single reload. |
...
| Feature | Notes |
| ------- | ----- |
| `doveadm_allowed_commands` setting | The setting only matched the command name, while the commands it was typically used to allow already gave full access to any user's mails. Dovecot fails to start if this setting is present in the configuration. Give doveadm server access only to trusted clients. The [[link,doveadm_http_api]] no longer returns the `403` HTTP response code, which was used only for commands rejected by this setting. |
| `shutdown_clients` setting | Replaced by [[setting,service_shutdown_clients_timeout]]. |
